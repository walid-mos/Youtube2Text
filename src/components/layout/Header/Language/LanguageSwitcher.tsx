...
const LanguageSwitcher: React.FC<Props> = ({ lang }) => {
	...
	const [isLanguageMenuOpen, setIsLanguageMenuOpen] = useAtom(
		isLanguageMenuOpenAtom,
	)
	const languageDropdownRef = useRef(null)

	const localeObject = languagesArray.find(
		({ code }) => code === lang,
	) as LanguagesType
	...
	useEffect(() => {
		...
		function handleClick(event: MouseEvent) {
			if (
				...
				!(languageDropdownRef.current as HTMLElement).contains(
	...
	return (
		<div className="relative hidden pb-5 md:block">
			<button
				type="button"
				className="flex items-center py-2 pl-5 pr-3 text-gray-500 bg-white rounded shadow focus:outline-none"
				onClick={() => setIsLanguageMenuOpen(get => !get)}
			>
				<span className="pr-4">{localeObject.icon}</span>
				<ChevronDownIcon size="s" />
			</button>
			{isLanguageMenuOpen && (
				<div
					className="absolute top-0 right-0 z-30 w-48 min-w-full mt-12 text-sm text-gray-700 bg-white rounded shadow-md"
					...
					<div className="relative z-10 w-full overflow-auto bg-white rounded">
						<ul className="list-reset">
							{languagesArray.map(language => {
								const isSelected =
									language.code === localeObject.code
								return (
